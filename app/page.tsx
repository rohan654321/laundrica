...
import {
  ...
  TbMist,
  TbShoe,
  TbLayoutGrid,        // ✅ instead of TbGrid
  TbBuilding,
  TbBuildingStore,
  TbCalendarEvent,
  TbBriefcase,         // ✅ instead of TbOffice
  TbCircleCheck,       // ✅ instead of TbCheckCircle
  TbStar,
  TbMessageCircle,
  TbArrowRight,
  TbShirt
...
// Trust Strip Component - Icons only
function TrustStrip() {
  const trustItems = [
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap justify-between items-center gap-6">
          {trustItems.map((item, index) => (
            <div key={index} className="flex items-center gap-2 text-sm font-medium text-[#00261b]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((step, idx) => (
            <motion.div
              ...
              className="group p-8 sm:p-10 rounded-2xl bg-[#f9faf7] border border-gray-100 hover:border-emerald-200 transition-all duration-300 hover:shadow-xl"
            >
              <div className="w-14 h-14 bg-[#bcedd7] rounded-xl flex items-center justify-center mb-8 group-hover:scale-110 transition-transform">
                <step.icon className="text-[#00261b] text-2xl" />
              </div>
              <h3 className="text-xl sm:text-2xl font-semibold text-[#00261b] mb-4">{step.title}</h3>
              <p className="text-[#5c5f5e] leading-relaxed">{step.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
...
// Services Grid Component - Updated with your actual services
              ...
              <Link href={`/services/${service.slug}/orders`}>
                ...
                <div className="p-6 md:p-8">
                  <div className="flex items-center gap-3 mb-4">
                    <service.icon className="text-emerald-700 text-2xl" />
                    <h3 className="text-xl font-bold text-[#00261b]">{service.name}</h3>
                  </div>
                  <p className="text-[#5c5f5e] text-sm line-clamp-3">{service.description}</p>
                </div>
              </Link>
            </motion.div>
          ))}
    ...
    <section ref={sectionRef} className="py-20 bg-[#00261b] text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <motion.div
          ...
          transition={{ duration: 0.6 }}
        >
          <span className="text-[#bcedd7] font-semibold tracking-wider uppercase text-sm mb-6 block">B2B Partnerships</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-10">
            {features.map((feature) => (
              <div key={feature.title} className="flex items-start gap-3">
                <feature.icon className="text-[#bcedd7] text-xl" />
                <div>
                  <p className="font-bold">{feature.title}</p>
                  <p className="text-sm text-[#79a894]">{feature.desc}</p>
                </div>
              </div>
            ))}
          </div>
          <button className="px-8 py-3.5 bg-white text-[#00261b] rounded-xl font-bold hover:bg-[#bcedd7] transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          ...
          >
            {benefits.map((benefit) => (
              <div key={benefit.text} className="space-y-2">
                <p className="font-bold text-[#00261b] flex items-center gap-2">
                  <TbCircleCheck className="text-emerald-600" />
                  {benefit.text}
                </p>
                <p className="text-sm text-[#5c5f5e]">{benefit.desc}</p>
              </div>
            ))}
          </motion.div>
        </div>
    ...
    <section ref={sectionRef} className="py-20 bg-[#edeeeb]" id="about">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <div className="lg:w-1/2">
            <h2 className="text-3xl sm:text-4xl font-bold text-[#00261b] mb-6">Redefining Fabric Care in Dubai</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          {testimonials.map((testimonial, idx) => (
            <motion.div
              ...
              animate={isInView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.5, delay: idx * 0.1 }}
              className="p-6 md:p-8 rounded-2xl bg-[#f9faf7] border border-gray-100 flex flex-col"
            >
              <div className="flex text-emerald-600 mb-5 gap-1">
                {[...Array(5)].map((_, i) => (
                  <TbStar key={i} className="fill-emerald-600" />
                ))}
              </div>
              <p className="text-[#00261b] font-medium mb-6 flex-grow italic">{testimonial.text}</p>
              <div>
                <p className="font-bold text-[#00261b]">{testimonial.name}</p>
                <p className="text-sm text-[#5c5f5e]">{testimonial.role}</p>
              </div>
            </motion.div>
          ))}
        </div>
    ...
    <section ref={sectionRef} className="py-20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ...
          transition={{ duration: 0.6 }}
          className="bg-[#00261b] rounded-3xl p-10 sm:p-16 text-center relative overflow-hidden"
        >
          ...
          <div className="relative z-10">
            <h2 className="text-3xl sm:text-4xl md:text-5xl text-white font-bold mb-6">Book Your Service Today</h2>
            <p className="text-[#79a894] text-lg mb-10 max-w-xl mx-auto">Experience the invisible luxury of professional fabric care. Fast, reliable, and just a click away.</p>
    ...
    <section ref={heroRef} className="relative min-h-[800px] lg:min-h-[921px] flex items-center overflow-hidden pt-10 pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <motion.div
          ...
          className="z-10"
        >
          <span className="inline-block px-4 py-1.5 bg-[#bcedd7] text-[#214f3f] rounded-full text-sm mb-6">Premium Care in Dubai</span>
          ...
          <div className="flex flex-wrap gap-4 mb-10">
            <Link href="/booking">
              <button className="px-6 sm:px-8 py-3.5 bg-[#00261b] text-white rounded-xl font-semibold hover:opacity-90 transition">
                Online Booking
              </button>
            </Link>
            <a href="https://wa.me/971XXXXXXXXX" target="_blank" rel="noopener noreferrer">
              <button className="px-6 sm:px-8 py-3.5 bg-white border border-gray-200 text-[#00261b] rounded-xl font-semibold hover:bg-gray-50 transition flex items-center gap-2">
        ...
        >
          ...
          <div className="absolute -bottom-5 -left-5 bg-white p-5 rounded-2xl shadow-xl max-w-[180px]">
...
export default function Home() {
  return (
    <main className="bg-[#f9faf7] text-[#1a1c1b] font-['Inter']">
      <Header />
      <HeroSection />
      <TrustStrip />
      <HowItWorks />

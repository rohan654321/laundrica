// app/services/page.tsx - Premium Redesign
'use client';
...
const SERVICE_IMAGES: Record<string, string> = {
  ...
  'shoe-cleaning': 'https://lh3.googleusercontent.com/aida-public/AB6AXuDP4Fl1mcL71ms-0aDco1bos4KJFJEZx5OQnJIOBWlWluJMOTU3XhoRrxAQvDa2yackx6UDbMN2aeY0HX3vJrKxXZZvOxDngQlZRCw6IC4qNlNInPtj13VA8r8kN-3-D0Jxrt44nDI5JAqB96hap1m-Sa8t_3oq6LroL8Ag9vqpd38eVyGtjT49EfXZUUUEmpo9H8CJRr1964I_IjjhCECEuvi3KYyJIWnmLx6Um420B0z6GG4nyB3DmF5ORn7DB6p1qP9FsFWioqH3',
  'carpet-cleaning': 'https://lh3.googleusercontent.com/aida-public/AB6AXuA45R7mYBYS5a_-9pcT1PdHvi75OrnW6SgurkHhCX-fJ_ymGD8x0ZZuqiCo5Rh908iElZhzYe3KP3UjAx1wUQ4w_Gkwp_0eqEpz_6SyRfDVW2dl0ja2MyCknffKUydongro0YT2wxCiDPDXKNyOovJkMUoqOJr4ZA-NfMZLhrLsPSzz1PycN1W0-fHxB0FkSehzYFH-4oAoWgJiJMcL_xJ9Sn_AkpjsLYsdhmJFybGq8Ju4kHUF-wp0f_OGW_HJf2FKgFPDwm8vZA6D',
};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
                ...
                <motion.div key={selectedService._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="bg-white rounded-2xl shadow-md overflow-hidden border border-gray-100">
                  ...
                  <div className="relative h-48 bg-cover bg-center" style={{ backgroundImage: `url('${getServiceImage(selectedService)}')` }}>
                    ...
                    <div className="relative z-10 p-6 h-full flex flex-col justify-end">
                      <div className="flex items-center gap-2 mb-2"><div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-lg">{getCategoryIcon(selectedService.category, "w-5 h-5")}</div>{selectedService.isFeatured && <span className="bg-amber-500 text-white text-xs px-2 py-1 rounded-full"><Star className="w-3 h-3 inline fill-white mr-1" />Featured</span>}</div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mb-6"><h3 className="font-semibold text-[#00261b] mb-3">Why choose this service?</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-2">{['Professional cleaning techniques', 'Premium eco-friendly products', 'Stain removal specialists', 'Quality checked before return', 'Free pickup & delivery', '24/7 customer support'].map((b, i) => (<div key={i} className="flex items-center gap-2 text-sm text-gray-600"><CheckCircle2 className="w-4 h-4 text-emerald-600" /><span>{b}</span></div>))}</div></div>

                    <div className="border-t border-gray-100 pt-6">
                      <div className="flex flex-wrap gap-3 mb-4"><div className="bg-[#f9faf7] rounded-xl p-1 flex"><button onClick={() => setActiveTab('website')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'website' ? 'bg-white text-[#00261b] shadow-sm' : 'text-gray-500'}`}><span className="text-lg">🌐</span> Book Online</button><button onClick={() => setActiveTab('whatsapp')} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${activeTab === 'whatsapp' ? 'bg-white text-[#00261b] shadow-sm' : 'text-gray-500'}`}><MessageCircle className="w-4 h-4" /> WhatsApp</button></div></div>
                      {activeTab === 'website' ? (<Link href={`/services/${selectedService.slug}/orders`}><button className="w-full md:w-auto bg-[#00261b] text-white px-8 py-3 rounded-xl font-semibold hover:bg-emerald-800 transition-all flex items-center justify-center gap-2">Book Now <ArrowRight className="w-4 h-4" /></button></Link>) : (<button onClick={handleWhatsAppOrder} className="w-full md:w-auto bg-[#25D366] text-white px-8 py-3 rounded-xl font-semibold hover:opacity-90 transition-all flex items-center justify-center gap-2"><MessageCircle className="w-4 h-4" /> Order on WhatsApp</button>)}
                      {activeTab === 'whatsapp' && (<div className="mt-4 p-3 bg-emerald-50 rounded-xl flex items-start gap-2"><Heart className="w-4 h-4 text-emerald-600" /><p className="text-xs text-emerald-800">Order via WhatsApp and get personalized assistance with pricing, scheduling, and special requirements.</p></div>)}
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            {services.length > 1 && (<div className="mt-8"><h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-4">Other Services You Might Like</h3><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{services.filter(s => s._id !== selectedService?._id).slice(0, 4).map((service) => (<button key={service._id} onClick={() => setSelectedService(service)} className="flex items-center gap-3 p-3 bg-white rounded-xl border border-gray-100 hover:shadow-md transition-all text-left group"><div className="w-10 h-10 bg-[#bcedd7] rounded-xl flex items-center justify-center">{getCategoryIcon(service.category, 'w-4 h-4')}</div><div className="flex-1"><p className="font-medium text-[#00261b] text-sm group-hover:text-emerald-700">{service.name}</p><p className="text-xs text-gray-400">{getCategoryLabel(service.category)}</p></div><ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-emerald-600" /></button>))}</div></div>)}
          </div>
        </div>
      </div>

      {/* Trust Bar */}
      <section className="bg-white border-t border-gray-100 py-8"><div className="max-w-7xl mx-auto px-4"><div className="flex flex-wrap justify-center gap-8 md:gap-12">{[{ icon: Truck, title: 'Free Pickup', sub: 'From your doorstep' }, { icon: Clock, title: '24-48hr Service', sub: 'Quick turnaround' }, { icon: Shield, title: '100% Safe', sub: 'Quality assured' }, { icon: MessageCircle, title: '24/7 Support', sub: 'WhatsApp & Call' }].map((item) => (<div key={item.title} className="flex items-center gap-3"><div className="w-10 h-10 bg-[#bcedd7] rounded-xl flex items-center justify-center"><item.icon className="w-5 h-5 text-[#00261b]" /></div><div><p className="font-semibold text-[#00261b] text-sm">{item.title}</p><p className="text-xs text-gray-400">{item.sub}</p></div></div>))}</div></div></section>
